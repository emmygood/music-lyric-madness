/* jshint esversion: 11 */

// Array, with objects and arrays inside, of all the questions
const questions = [
    {
        question: "Which of these songs by artist Eminem contain the lyric 'Nice to meet you. Hi, my name is... I forgot my name'?",
        answers: [
            {text: "Rain Man", correct: true},
            {text: "Without Me", correct: false},
            {text: "Kim", correct: false},
            {text: "Square Dance", correct: false}
        ]
    },
    {
        question: "What was the best selling album of 2015?",
        answers: [
            {text: "Fetty Wap, Fetty Wap", correct: false},
            {text: "Adele, 25", correct: true},
            {text: "Taylor Swift, 1989", correct: false},
            {text: "Justin Bieber, Purpose", correct: false}
        ]
    },
    {
        question: "Who is the lead singer of Arctic Monkeys?",
        answers: [
            {text: "Jamie Cook", correct: false},
            {text: "Matt Helders", correct: false},
            {text: "Nick O'Malley", correct: false},
            {text: "Alex Turner", correct: true}
        ]
    },
    {
        question: "Which member of the Foo Fighters was previously the drummer for Nirvana?",
        answers: [
            {text: "Taylor Hawkins", correct: false},
            {text: "Dave Grohl", correct: true},
            {text: "Nate Mendel", correct: false},
            {text: "Chris Shiflett", correct: false}
        ]
    },
    {
        question: "Johnny Cash did a cover of this song written by lead singer of Nine Inch Nails, Trent Reznor.",
        answers: [
            {text: "Closer", correct: false},
            {text: "A Warm Place", correct: false},
            {text: "Hurt", correct: true},
            {text: "Big Man with a Gun", correct: false}
        ]
    },
    {
        question: "Which member of the British pop group 'The Spice Girls'; was known as Ginger Spice?",
        answers: [
            {text: "Melanie Brown", correct: false},
            {text: "Geri Halliwell", correct: true},
            {text: "Emma Bunton", correct: false},
            {text: "Victoria Beckham", correct: false}
        ]
    },
    {
        question: "Which of these aliases has NOT been used by electronic musician Aphex Twin?",
        answers: [
            {text: "Caustic Window", correct: false},
            {text: "Bradley Strider", correct: false},
            {text: "GAK", correct: false},
            {text: "Burial", correct: true}
        ]
    },
    {
        question: "What was the title of ABBA's first UK hit single?",
        answers: [
            {text: "Waterloo", correct: true},
            {text: "Mamma Mia", correct: false},
            {text: "Fernando", correct: false},
            {text: "Dancing Queen", correct: false}
        ]
    },
    {
        question: "The heavy metal band Black Sabbath hail from which English city?",
        answers: [
            {text: "Newcastle-Upon-Tyne", correct: false},
            {text: "London", correct: false},
            {text: "Manchester", correct: false},
            {text: "Birmingham", correct: true}
        ]
    },
    {
        question: "What was Rage Against the Machine's debut album?",
        answers: [
            {text: "Evil Empire", correct: false},
            {text: "Bombtrack", correct: false},
            {text: "Rage Against the Machine", correct: true},
            {text: "The Battle Of Los Angeles", correct: false}
        ]
    }
]